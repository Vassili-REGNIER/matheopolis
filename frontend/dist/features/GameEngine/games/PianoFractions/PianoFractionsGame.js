...
export class PianoFractionsGame extends BaseGame {
    ...
    message = "";
    messageTone = "good";
    missionHint = "Etape 1 : reduis la fraction. Etape 2 : trouve sa quinte.";
    audioContext = null;
    start() {
        this.renderGame();
    }
    ...
    renderGame(activeNote = "") {
      ...
      ${this.style()}
    `;
        this.container.querySelectorAll("[data-note]").forEach((button) => {
        ...
        const melody = this.container.querySelector('[data-action="melody"]');
        if (melody !== null) {
            this.listen(melody, "click", () => this.playMelody());
        }
        const restart = this.container.querySelector('[data-action="restart"]');
        if (restart !== null) {
    ...
    playFrequency(frequency, duration = 0.42) {
