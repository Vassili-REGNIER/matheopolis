...
export class GameContainerComponent extends BaseComponent {
    ...
    score = 0;
    ending = false;
    constructor(container, router, services, riddleId) {
    ...
    bindEvents() {
        const back = this.query(".back-button");
        if (back !== null) {
            this.listen(back, "click", () => this.router.navigate("/game-home"));
        }
        const blockHost = this.query(".block-host");
        if (blockHost !== null) {
    ...
    mountBlock(step) {
        const host = this.query(".block-host");
        if (host === null) {
            return;
        }
        this.currentBlock?.destroy();
