...
const navItems = [
    ...
    { id: "quiz-management", label: "Gestion des jeux", icon: "settings", roles: ["teacher"] },
    { id: "admin", label: "Administration", icon: "graduation", roles: ["admin"] }
];
export class NavigationComponent extends BaseComponent {
    role;
    activeView;
    constructor(container, role, activeView) {
    ...
    bindEvents() {
        this.queryAll("[data-view]").forEach((button) => {
        ...
        const gameButton = this.query('[data-action="game"]');
        if (gameButton !== null) {
            this.listen(gameButton, "click", () => this.emit("panel:game"));
        }
        const logoutButton = this.query('[data-action="logout"]');
