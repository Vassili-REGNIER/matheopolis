...
export class GameHomeComponent extends BaseComponent {
    ...
    bindEvents() {
        this.queryAll("[data-route]").forEach((button) => {
        ...
        this.queryAll("[data-riddle-id]").forEach((card) => {
    ...
    async load() {
        ...
        const progressPairs = await Promise.all(puzzles.map(async (puzzle) => ({
            puzzle,
